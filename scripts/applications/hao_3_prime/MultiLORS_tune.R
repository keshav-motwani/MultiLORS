...
MultiLORS_fit = readRDS(file.path(result_path, "MultiLORS_tuning.rds"))

MultiLORS_refit = refit_MultiLORS(MultiLORS_fit, prepared_train$Y_list, X_list[train_data], prepared_train$indices_list,
                                  prepared_validation$Y_list, X_list[validation_data], prepared_validation$indices_list, n_cores = 10)

saveRDS(MultiLORS_refit, file.path(result_path, "MultiLORS_refit_tuning.rds"))